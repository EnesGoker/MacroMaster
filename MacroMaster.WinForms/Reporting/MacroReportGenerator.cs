...
internal static class MacroReportGenerator
{
    ...
    public static MacroReportStatistics Analyze(MacroSession session, string? filePath)
    {
        ...
        List<MacroEvent> events = session.Events;
    ...
    private static IEnumerable<string> BuildTextEventRows(List<MacroEvent> events)
    {
        if (events.Count == 0)
        {
    ...
    private static string BuildHtmlEventRows(List<MacroEvent> events)
    {
        if (events.Count == 0)
        {
            return """
